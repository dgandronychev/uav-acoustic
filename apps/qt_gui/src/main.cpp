...
int main(int argc, char *argv[]) {
  ...
  std::thread pub([&]{
    ...
    while (running.load()) {
      ...
      // p_detect mock: smooth sinus + occasional spikes imitation
      float p = static_cast<float>(0.5 + 0.45 * std::sin(t));
      if (static_cast<int>(t) % 7 == 0) p = std::min(1.0f, p + 0.25f);

      // FSM mock (позже заменим на реальную FSM)
      ...
      // Timeline
      s->timeline_n = 64;
      const std::int64_t base = s->t_ns - 64 * 250'000'000LL; // roughly 0.25s steps
